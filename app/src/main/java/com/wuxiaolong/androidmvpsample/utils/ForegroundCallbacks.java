...
// 使用方法：（在自定义的application中的onCreate方法中执行如下方法）
...
public class ForegroundCallbacks implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityResumed(Activity activity) {
        ...
        if (wasBackground) {
//            Log.e("app","went foreground");
            for (Listener l : listeners) {
                try {
                    l.onBecameForeground();
                } catch (Exception exc) {
                    exc.printStackTrace();
//                    Log.e("app","Listener threw exception!");
                }
            }
        } else {
//            Log.e("app","still foreground");
        }
    }
        ...
        handler.postDelayed(check = new Runnable() {
            ...
            public void run() {
                if (foreground && paused) {
                    foreground = false;
//                    Log.e("app","went background");
                    for (Listener l : listeners) {
                        try {
                            l.onBecameBackground();
                        } catch (Exception exc) {
                            exc.printStackTrace();
//                            Log.e("app","Listener threw exception!", exc);
                        }
                    }
                } else {
//                    Log.e("app","still foreground");
